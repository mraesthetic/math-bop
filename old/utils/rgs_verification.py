...
def verify_books_and_payout_mults(books_filename: str) -> list:
    ...
    book_payout_dict = {}  # Store as dict with id as key to sort later
    total_num_events = 0
    with open(books_filename, "rb") as f:
        ...
        with decompressor.stream_reader(f) as reader:
            ...
            for line in txt_stream:
                ...
                # payoutMultiplier is already an integer in cents (100x format)
                book_payout_dict[blob["id"]] = int(blob["payoutMultiplier"])

    # Sort by ID to match lookup table order
    book_payout_ints = [book_payout_dict[sim_id] for sim_id in sorted(book_payout_dict.keys())]
