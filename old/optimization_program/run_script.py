...
class OptimizationExecution:
    ...
    def run_opt_single_mode(game_config, mode, threads):
        ...
        # Try to load math_config.json if it exists, but don't fail if it doesn't
        # We'll use game_config.opt_params instead
        ...
        try:
            ...
            # File doesn't exist yet, that's okay - we'll use game_config.opt_params
            pass
    ...
    def run_rust_script():
        ...
        if result.returncode == 0:
            ...
            print("="*80)
            print("ERROR in optimization program (Rust)")
            print("="*80)
            print("STDOUT:")
            print(result.stdout)
            print("\nSTDERR:")
            print(result.stderr)
            print("="*80)
            raise RuntimeError(f"Optimization program failed with exit code {result.returncode}")
