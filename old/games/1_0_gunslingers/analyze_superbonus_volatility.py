#!/usr/bin/env python3
"""
Analyze SUPERBONUS feature volatility and win distribution.

This script:
1. Runs 100k superbonus feature simulations (if needed)
...
def generate_markdown_report(stats, output_file=None):
    ...
    report = f"""# SUPERBONUS Feature Volatility Analysis
...
|--------|----------|------------|
| **< 25×** | {stats['bucket_counts']['< 25×']:,} | {stats['bucket_percentages']['< 25×']:.2f}% |
...
| **500–1000×** | {stats['bucket_counts']['500–1000×']:,} | {stats['bucket_percentages']['500–1000×']:.2f}% |
| **1000×+** | {stats['bucket_counts']['1000×+']:,} | {stats['bucket_percentages']['1000×+']:.2f}% |
...
| **≥ 800×** | {stats['above_800_count']:,} | {stats['above_800_pct']:.2f}% |
| **≥ 1000×** | {stats['above_1000_count']:,} | {stats['above_1000_pct']:.2f}% |
...
- **1000×+ Hits**: {stats['above_1000_pct']:.2f}% of features ({stats['above_1000_count']:,} features)
...
def main():
    ...
    print("\n" + "="*80)
    print("SUPERBONUS VOLATILITY ANALYSIS")
    print("="*80)
    ...
    output_file = os.path.join(
        ...
        "SUPERBONUS_VOLATILITY_REPORT.md"
    )
    
    generate_markdown_report(stats, output_file=output_file)
